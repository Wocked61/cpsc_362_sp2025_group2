...
function createColorPickers() {
  ...
  const colorControlsContainer =
    document.getElementById("board-customization") ||
...
function canCaptureKing(startRow, startCol, kingRow, kingCol) {
  ...
  switch (piece.type) {
    ...
    case "queen":
      return (
        canStraightCapture(startRow, startCol, kingRow, kingCol) ||
...
function validateQueenMove(startRow, startCol, endRow, endCol) {
  return (
    validateStraightMove(startRow, startCol, endRow, endCol) ||
...
function validMove(startRow, startCol, endRow, endCol) {
  ...
  switch (piece.type) {
    case "pawn":
      isValidPieceMove = validatePawnMove(
        startRow,
        startCol,
        endRow,
        endCol,
      ...
      break
    case "rook":
      isValidPieceMove = validateStraightMove(
        startRow,
        startCol,
        endRow,
        endCol,
    ...
    case "knight":
      ...
      break
    case "bishop":
      isValidPieceMove = validateDiagonalMove(
        startRow,
        startCol,
        endRow,
        endCol,
...
function handleClick(event) {
  ...
  if (selectedPiece) {
    if (selectedPiece.row === clickedRow && selectedPiece.col === clickedCol) {
      getSquare(selectedPiece.row, selectedPiece.col).classList.remove(
        "selected",
      )
      selectedPiece = null
      return
    ...
    if (
      handleMove(selectedPiece.row, selectedPiece.col, clickedRow, clickedCol)
    ) {
      getSquare(selectedPiece.row, selectedPiece.col).classList.remove(
        "selected",
      )
...
function handleMove(startRow, startCol, endRow, endCol) {
  ...
  const isPawnDoubleMove =
    piece.type === "pawn" && Math.abs(endRow - startRow) === 2
...
function promotePawn(row, col) {
  ...
  pieceTypes.forEach((type) => {
    ...
    pieceContainer.addEventListener("click", () => {
      ...
      newPiece.addEventListener("touchstart", handleTouchStart, {
        passive: false,
      })
      newPiece.addEventListener("touchmove", handleTouchMove, {
        passive: false,
      })
...
function endGame() {
  ...
  const newGameBtn =
    document.getElementById("new-game-btn") || createNewGameButton()
  if (newGameBtn) {
...
function highlightMove(from, to) {
  document
    .querySelectorAll(".square.highlight-from, .square.highlight-to")
    .forEach((sq) => {
...
function saveSettings() {
  ...
  const styleElement =
    document.getElementById("piece-colors-style") ||
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (!document.getElementById("timer")) {
    ...
    const controlsContainer =
      document.getElementById("game-controls") || document.body
